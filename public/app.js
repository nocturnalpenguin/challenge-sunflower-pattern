...
var CanvasBase = /*#__PURE__*/function () {
  ...
  _createClass(CanvasBase, [{
    ...
    key: "preRenderCanvas",
    value: function preRenderCanvas(width, height) {
      var callback = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : function (ctx) {};
...
var Sunflower = /*#__PURE__*/function (_CanvasBase) {
  _inherits(Sunflower, _CanvasBase);

  var _super = _createSuper(Sunflower);

  function Sunflower() {
    var _this;

    _classCallCheck(this, Sunflower);
    ...
    _this.divergenceAngle = 137.5;
    _this.divergenceRadian = _this.divergenceAngle * Math.PI / 180;
    _this.scale = 9 * _this.ratio;
    _this.numberOfSeeds = 950;
    _this.seedDiameter = 5 * _this.ratio;
    _this.hueOffset = 297;
    _this.zoomLevel = 1;
    _this.points = [];
    _this.useDots = true;
    _this.resizeCallbacks = [_this.restartAnimation.bind(_assertThisInitialized(_this))];
    return _this;
  }

  _createClass(Sunflower, [{
    key: "init",
    value: function init() {
      this.attachControls();
    ...
    value: function calculatePoints() {
      ...
      for (var n = 1; n <= this.numberOfSeeds; n++) {
        var phi = n * this.divergenceRadian;
        var r = this.zoomLevel * this.scale * Math.sqrt(n);
        var x = r * Math.cos(phi);
        var y = r * Math.sin(phi) * -1;
        var d = this.distanceFromCenter(centerX + x, centerY + y); // const fillStyle = this.colorEasing(d)

        var fillStyle = this.colorEasing(n);
        this.points.push({
          x: centerX + x,
          y: centerY + y,
          fillStyle: fillStyle
    ...
    key: "colorEasing",
    value: function colorEasing(n) {
      var hue = (n * this.divergenceAngle + this.hueOffset) % 361;
      return "hsl(".concat(hue, ", 85%, 60%)");
    }
  }, {
    key: "drawPattern",
    value: function drawPattern() {
      if (this.useDots) {
        for (var i = 0, pointLen = this.points.length; i < pointLen; i++) {
        ...
        for (var _i = 1, _pointLen = this.points.length; _i < _pointLen; _i++) {
          this.ctx.beginPath();
          this.ctx.strokeStyle = this.points[_i].fillStyle;
          this.ctx.moveTo(this.points[_i - 1].x, this.points[_i - 1].y);
          this.ctx.lineTo(this.points[_i].x, this.points[_i].y);
          this.ctx.stroke();
        }
      }
    }
    ...
    value: function distanceFromCenter(x, y) {
      var centerX = this.canvas.width / 2;
      var centerY = this.canvas.height / 2;
    ...
    value: function attachControls() {
      ...
      var radioInputs = document.querySelectorAll('#controls input[type="radio"]');

      for (var i = 0, rangeInputsLen = rangeInputs.length; i < rangeInputsLen; i++) {
      ...
      for (var _i2 = 0, radioInputsLen = radioInputs.length; _i2 < radioInputsLen; _i2++) {
        radioInputs[_i2].addEventListener('click', this.updateInputValueLabel.bind(this));
      }

      this.updateInputs([].concat(_toConsumableArray(rangeInputs), _toConsumableArray(radioInputs)));
    }
  }, {
    key: "updateInputs",
    value: function updateInputs(inputs) {
      for (var i = 0; i < inputs.length; i++) {
        var name = inputs[i].getAttribute('name');

        if (inputs[i].getAttribute('type') === 'range') {
          document.querySelector("#controls [data-for=\"".concat(name, "\"]")).innerText = this[name];
    ...
    key: "controlListener",
    value: function controlListener(e) {
    ...
    value: function updateInputValueLabel(e) {
      ...
      if (e.target.getAttribute('type') !== 'radio') {
        document.querySelector("#controls [data-for=\"".concat(name, "\"]")).innerText = e.target.value;
      }
...
__webpack_require__(/*! /Users/nikmax/code/challenges/sunflower/src/js/app.js */"./src/js/app.js");
module.exports = __webpack_require__(/*! /Users/nikmax/code/challenges/sunflower/src/css/app.scss */"./src/css/app.scss");
